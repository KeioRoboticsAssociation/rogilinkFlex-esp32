...
// 独自のindex_sequenceの実装
template<size_t... Indices>
struct index_sequence {};

// 独自のmake_index_sequenceの実装を行うためのヘルパーテンプレート
...
// 独自のmake_index_sequenceの実装
template<size_t N>
using make_index_sequence = typename make_index_sequence_impl<N>::type;

// 独自のindex_sequence_forの実装
template<typename... T>
...
typename std::enable_if<std::is_same<T, char*>::value>::type
deserialize(const uint8_t* buffer, T* data, size_t* size){
    memcpy(data, buffer, strlen((char*)buffer) + 1);
...
class TupleDeserializer{
    ...
    int deserialize_tuple(const uint8_t* buffer, std::tuple<Args...>& tuple, size_t len) {
        return deserialize_tuple_impl(buffer, typename index_sequence_for<Args...>::type{}, len, tuple);
    }

    template<std::size_t... I>
    int deserialize_tuple_impl(const uint8_t* buffer, index_sequence<I...>, size_t len, std::tuple<Args...>& tuple) {
        size_t offset = 0;
