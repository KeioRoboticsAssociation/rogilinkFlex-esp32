...
auto apply_impl(Function f, Tuple&& t, index_sequence<Indices...>) -> decltype(f(std::get<Indices>(std::forward<Tuple>(t))...)) {
    return f(std::get<Indices>(std::forward<Tuple>(t))...);
}

// std::applyの代替(C++17未満)
template<typename Function, typename Tuple>
auto apply(Function f, Tuple&& t) -> decltype(apply_impl(f, std::forward<Tuple>(t), make_index_sequence<std::tuple_size<typename std::remove_reference<Tuple>::type>::value>{})) {
    return apply_impl(f, std::forward<Tuple>(t), make_index_sequence<std::tuple_size<typename std::remove_reference<Tuple>::type>::value>{});
}
